import { ref, uploadBytes, getStream, getBytes } from "firebase/storage";
import storage from "./index.js";
import fs from "fs";

function upload(fileName, file, space) {
    const fileWithLocationRef = ref(storage, `${space ? space + "/" : ""}${fileName}`);
    uploadBytes(fileWithLocationRef, file).then(() => console.log('Uploaded a blob or file: ', fileName));
}
